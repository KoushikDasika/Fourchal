Feature: Create New Challenge
  ...
  Scenario: A Signed In User adds a new Challenge to a new Competition
    ...
    And I add challenges to the competition form with the following values:
      | title | description | start | stop     | validation | badge | limit | prize    | points |
      | Run   | running     | now   | tomorrow | super      | green | 10    | stickers | 10     |
      | Walk  | walking     | now   | tomorrow | purple     | green | 10    | stickers | 10     |
      | Jump  | jumping     | now   | tomorrow | green      | green | 10    | stickers | 10     |
